...
//    setCameraID(quint32::CamIds[0]);
    ...
    Result=UCC_OpenCamera(auCamIds[1]);
    if(S_OK!=Result)
    {
    ...
    Result=UCC_GetCurrentImageFormat(auCamIds[1],&auimagewidth,&auimageheight,&Code,&depth);
    if(S_OK!=Result)
    {
    ...
    Result= UCC_GrabStart(auCamIds[1],1000);
    if(S_OK!=Result)
    {
...
{
    UCC_GetBitmapImage(auCamIds[1],UINT8P_CAST(aucameraframe.bits()),10);
}
