...
//    setCameraID(quint32::CamIds[0]);
    ...
    Result=UCC_OpenCamera(sbCamIds[0]);
    if(S_OK!=Result)
    {
    ...
    Result=UCC_GetCurrentImageFormat(sbCamIds[0],&sbimagewidth,&sbimageheight,&Code,&depth);
    if(S_OK!=Result)
    {
    ...
    Result= UCC_GrabStart(sbCamIds[0],1000);
    if(S_OK!=Result)
    {
...
{
    UCC_GetBitmapImage(sbCamIds[0],UINT8P_CAST(sbcameraframe.bits()),10);
}
